#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""CryptoBot GitHub Updater v2.2 — fixed push feedback."""
import os
import sys
import subprocess
import argparse
...
def push_changes(root: Path, force: bool = False, branch: str = None):
    ...
    if code == 0:
        ...
        if "rejected" in stderr.lower():
            print("   💡 Подсказка: используй Pull перед Push")
        elif "could not resolve" in stderr.lower():
            print("   💡 Проверь интернет-соединение и remote URL")
...
def show_menu(root: Path) -> str:
    ...
    print("\n" + "=" * 60)
    print("  🚀 CRYPTO BOT GITHUB UPDATER v2.2")
    print("=" * 60)
    ...
    if status["has_changes"]:
        print(f"  📋 Changes: {len(status['modified'])} modified, "
...
def main():
    parser = argparse.ArgumentParser(description="CryptoBot GitHub Updater")
    ...
    if not ensure_git_repo(root):
        sys.exit(1)
    has_cli = any([args.status, args.commit, args.push, args.pull, args.stash, args.add is not None, args.all, args.setup])
    if args.menu or not has_cli:
        handle_menu(root)
        return 0
