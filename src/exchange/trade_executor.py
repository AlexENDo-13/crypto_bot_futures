...
class TradeExecutor:
    ...
    def update_balance_sync(self) -> float:
        """Fetch balance synchronously (for GUI thread)."""
        if not self.paper and self.api and self.api.api_key:
            try:
                import asyncio
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                result = loop.run_until_complete(self.api.get_balance())
                loop.close()
                if isinstance(result, dict) and result.get("code") == 0:
                    ...
                    if isinstance(data, dict):
                        ...
                        if avail > 0:
                            self._balance = avail
                            self.logger.info("Balance updated from API: $%.2f", self._balance)
                    elif isinstance(data, list) and len(data) > 0:
                        asset = data[0]
                        avail = float(asset.get("available", asset.get("balance", 0)))
                        if avail > 0:
                            self._balance = avail
                            self.logger.info("Balance updated from API: $%.2f", self._balance)
            except Exception as e:
                self.logger.warning("Balance sync failed: %s", e)
        return self._balance
    ...
    async def update_balance(self) -> float:
        ...
        if not self.paper and self.api and self.api.api_key:
            try:
                ...
                self.logger.warning("Balance async failed: %s", e)
        return self._balance
