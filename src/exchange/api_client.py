"""
BingX API Client v9.2 - FIXED Authentication
Proper X-BX-APIKEY header + correct endpoints for BingX Swap API.
"""
import asyncio
import hashlib
import hmac
...
class BingXAPIClient:
    ...
    def update_credentials(self, api_key: str, api_secret: str):
        ...
        # Reset circuit breaker on credential update
        self._circuit_open = False
        self._consecutive_errors = 0
    ...
    async def _get_or_create_session(self) -> aiohttp.ClientSession:
        if self._session is None or self._session.closed:
            ...
            if self.api_key and "X-BX-APIKEY" not in self._session.headers:
                self._session.headers["X-BX-APIKEY"] = self.api_key
    ...
    async def _request(self, method: str, endpoint: str, params: Optional[dict] = None, 
        ...
        if signed:
            ...
            # apiKey also in query for some endpoints, but primary is X-BX-APIKEY header
        ...
        for attempt in range(retries):
            try:
                ...
                if data.get("code") == 100400 or "api is not exist" in msg:
                    ...
                    break  # Don't retry wrong endpoint

                if "null apikey" in msg or "unable to find api key" in msg:
                    last_error = data
                    self._consecutive_errors += 1
                    self.logger.error(f"API KEY NOT IN HEADER! Attempt {attempt+1}/{retries}")
                    # Recreate session with proper headers
    ...
    # Account / Trading (signed) - CORRECT ENDPOINTS
    async def get_account_balance(self) -> dict:
        # Try v2 first (more stable), then v3
        for endpoint in ["/openApi/swap/v2/user/balance", "/openApi/swap/v3/user/balance"]:
            response = await self._request("GET", endpoint, {}, signed=True)
            self.logger.info(f"Balance response ({endpoint}): {response}")
            if response.get("code") == 0 and "data" in response:
                return response["data"]
            if response.get("code") != 100400 and "api is not exist" not in str(response.get("msg", "")).lower():
                break  # Real error, not endpoint issue
        return {}
    ...
    async def get_positions(self, symbol: Optional[str] = None) -> list:
        ...
        for endpoint in ["/openApi/swap/v2/user/positions", "/openApi/swap/v3/user/positions"]:
    ...
    async def set_leverage(self, symbol: str, leverage: int, position_side: str = "BOTH") -> dict:
        ...
        response = await self._request("POST", "/openApi/swap/v3/trade/leverage", params, signed=True)
    ...
    async def set_margin_mode(self, symbol: str, margin_mode: str) -> dict:
        ...
        response = await self._request("POST", "/openApi/swap/v3/trade/marginMode", params, signed=True)
        if response.get("code") == 0:
            return response.get("data", {})
    ...
    async def place_order(self, symbol: str, side: str, position_side: str,
        ...
        response = await self._request("POST", "/openApi/swap/v3/trade/order", params, signed=True)
        if response.get("code") == 0:
            return response.get("data", {})
    ...
    async def cancel_order(self, symbol: str, order_id: str) -> bool:
        ...
        response = await self._request("DELETE", "/openApi/swap/v3/trade/order", params, signed=True)
        return response.get("code") == 0

    async def get_open_orders(self, symbol: str) -> list:
        params = {"symbol": symbol}
        response = await self._request("GET", "/openApi/swap/v3/trade/openOrders", params, signed=True)
        if response.get("code") == 0 and "data" in response:
