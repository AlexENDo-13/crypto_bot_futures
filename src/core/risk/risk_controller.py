...
class RiskController:
    def __init__(self, logger, settings):
        ...
        self._correlation_limit = settings.get("correlation_limit_enabled", True)

    def _reset_daily_if_needed(self):
        now = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
        if now > self._daily_reset_time:
            self.daily_pnl = self.daily_loss = 0.0
            self._daily_reset_time = now
            self._circuit_breaker_active = False
            self._position_count = 0
            self.logger.info("RiskController: daily stats reset")
    ...
    def filter_signals(self, signals, positions, balance):
        ...
        filtered = []
        total_risk = 0.0
        open_bases = {p.symbol.split("/")[0] for p in positions}
    ...
    def register_position_open(self, symbol):
        self._position_count += 1
        self._open_symbols.add(symbol)

    def register_position_close(self, symbol):
