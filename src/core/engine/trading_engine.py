...
class TradingEngine:
    ...
    def stop(self):
        ...
        self._pause_event.set()  # Снимаем паузу, чтобы поток завершился
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=5.0)
        self.logger.info("Торговый движок остановлен")

    def pause(self):
        """Приостановка торговли (не закрывает позиции)"""
    ...
    async def _main_loop(self):
        ...
        # Первоначальная синхронизация баланса и рисков
        await self._update_account_info()

        scan_interval = getattr(self.settings, 'scan_interval_seconds', 60)
        ...
        while not self._stop_event.is_set():
            ...
            try:
                # Обновление информации о счёте и рисках
                await self._update_account_info()
                ...
                # Обработка сигналов и отправка ордеров
                for signal in signals:
                    if self.state != EngineState.RUNNING or self._stop_event.is_set():
                        break
                    await self._process_signal(signal)

                # Управление открытыми позициями (трейлинг-стопы, частичная фиксация и т.д.)
                await self._manage_positions()

                # Уведомление UI об обновлении статуса
            ...
            # Вычисляем время до следующего сканирования
            elapsed = time.time() - cycle_start
            sleep_time = max(0, scan_interval - elapsed)
            # Делим сон на короткие интервалы, чтобы реагировать на stop/pause
    ...
    async def _update_account_info(self):
        ...
        try:
            # Получение баланса (предполагаем, что executor/risk_manager умеют)
    ...
    async def _manage_positions(self):
        ...
        for pos in positions:
            try:
                # Проверка необходимости трейлинг-стопа или частичной фиксации
                await self.risk_manager.manage_position_risk(pos)
            except Exception as e:
                self.logger.error(f"Ошибка управления позицией {pos.get('symbol')}: {e}")

    # Методы для совместимости с main_window.py
    def start_async(self):
        """Совместимость с возможным вызовом start_async"""
        self.start()

    def stop_async(self):
        """Совместимость с возможным вызовом stop_async"""
        self.stop()
